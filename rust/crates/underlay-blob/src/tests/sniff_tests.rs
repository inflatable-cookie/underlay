...
fn sniffs_known_signatures() {
    ...
    assert_eq!(sniff_content_type(b"\x00\x00\x00\x20ftypavif...."), Some("image/avif"));
...
fn declared_type_must_match_sniffed_signature() {
    ...
    assert!(content_matches_declared(PNG_MAGIC, "IMAGE/PNG; charset=binary"));

    // HTML payload labelled as an image must be rejected.
    assert!(!content_matches_declared(b"<html><script>alert(1)</script>", "image/png"));
    ...
    assert!(!content_matches_declared(b"not an image at all", "image/jpeg"));
}
...
fn non_sniffable_declared_types_pass_only_without_other_signatures() {
    ...
    assert!(content_matches_declared(b"<svg xmlns=...>", "image/svg+xml"));
