...
type LintResult = {
  schema: "underlay.migration.promotion_decision_lint.v1";
  schema_version: 1;
  generated_at: string;
...
function defaultSchemaPath(config: Record<string, string>): string {
  const configured = readOptional(config, "PROMOTION_DECISION_SCHEMA_FILE");
  if (configured.length > 0) {
    return resolve(configured);
  }
...
function defaultLintOutputPath(config: Record<string, string>): string {
  ...
  const runDate = readString(config, "RUN_DATE_UTC", utcDate());
  return resolve(
    `${outputDir}/metadata/${project}.${scope}.${runDate}.promotion-decision-lint.json`,
  );
}
...
function main(): void {
  ...
  const decisionPath = resolve(args.input || defaultDecisionPath(values as Record<string, string>));
  ...
  let schemaRoot: JsonSchema & { properties?: Record<string, JsonSchema> };

  if (!existsSync(decisionPath)) {
    errors.push(`decision file not found: ${decisionPath}`);
  }
  ...
  if (errors.length === 0) {
    ...
    schemaRoot = readJson(schemaPath) as JsonSchema & { properties?: Record<string, JsonSchema> };
    ...
    validateAgainstSchema(decision, rootSchema, "$", errors);
  }
  ...
  if (args.json) {
    ...
    console.log(`promotion decision lint ${status}: ${decisionPath}`);
  ...
  if (status === "failed") {
    const message = [
      `promotion decision lint failed (${errors.length} issue${errors.length === 1 ? "" : "s"})`,
      ...errors.map((error) => `- ${error}`),
    ].join("\n");
    fail("MIG_PROMO_002", message);
  }
}
