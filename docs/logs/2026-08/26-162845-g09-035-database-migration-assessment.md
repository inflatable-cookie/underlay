# g09.035 - Database Migration Contract Assessment
...
- Rejected alternative: restore `db:*` in contract `021`. That reverses
  `g09.022` and the chosen Acowtancy proof. Compatibility aliases are also
...
- Promotion: findings-driven roadmap wave after `g09.036`.
- Confidence: high.
...
- Promotion: bounded Acowtancy roadmap after `g09.036`.
...
- Promotion: bounded Songsprout and Composer repair roadmaps after `g09.036`.
- Confidence: high.
...
by `g09.022`, and three replay paths can report success without fully applying
...
Execute `g09.036`, the read-only testing posture assessment. Then compile one
