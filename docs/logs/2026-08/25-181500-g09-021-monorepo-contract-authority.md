# g09.021 — Monorepo Contract Authority

Date: 2026-08-25
Roadmap: `docs/roadmaps/g09/021-monorepo-contract-authority.md`
...
- Second-tier guide cleanup (`g09.022`) and conformance code (`g09.023`) were
  not started.
- No closed roadmap, log, handoff, or frozen migration record was rewritten.
- Acowtancy's obsolete README `file:` prose was left in place; it is `g09.024`.
...
3. **Stale routing on changed front doors.** `docs/roadmaps/README.md` still
   said to execute the handoff; `docs/roadmaps/g09/README.md` still called Lane
   A the likely `g09.021` candidate and treated the roadmap count as open despite
   the compiled ten-roadmap runway; `docs/guides/000-overview.md` still routed new
   ...
   not-current warning until `g09.022` normalizes that guide.
...
an unrelated contract, outside `g09.021`'s scope. Left untouched and flagged for
the orchestrator to roadmap.
...
The contract `021` adjacency is promoted into `g09.022` as a bounded
prerequisite to the active-guide sweep. No `docs/triage/` notes existed for this
run.

## Next Task

Execute `g09.022` — migration contract and active guide normalization — through
its orchestrator-dispatched worker handoff. No other roadmap is ready.
