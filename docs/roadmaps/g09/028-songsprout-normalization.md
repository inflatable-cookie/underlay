# g09.028 - Songsprout Normalization
...
- Correct the reversed Effigy role mapping: `stem` is the client and `petal`
  is the UI package.
- Replace per-package bundle hydration with the `g09.025` single frozen root
  install pattern.
...
confirm the roadmap's intended correction. The root JavaScript workspace has four
...
Worker handoff
`docs/handoffs/20260826-112215-g09-028-songsprout-normalization.md` is published
...
Complete `g09.030` fleet proof and closeout.
