...
- legacy nested `Paginated<T>` remains documented as a distinct compatibility
  shape; no runtime response changes land in this roadmap
...
That would require a consumer cutover roadmap, not an artifact-only repair.
...
Execute `g09.034`, the bounded HTTP-client constructor fallback repair.
