...
- Name Acowtancy as current live proof and Underlay Reference as the fixture
  that must converge in `g09.025`.
- Keep active library-facing docs free of absolute local paths.

## Out Of Scope

- Second-tier guide cleanup owned by `g09.022`.
- Conformance implementation owned by `g09.023`.
...
findings. Both predate this roadmap.
...
orchestrator to roadmap. It is now the first bounded repair in `g09.022`, before
...
currentness repair. Continue through a fresh `g09.022` worker handoff.

## Next Task

Execute `g09.022` — migration contract and active guide normalization. No other
roadmap is ready.
