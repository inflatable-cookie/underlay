---
title: g09.021 monorepo contract authority worker handoff
...
created: 2026-08-25
updated: 2026-08-25
handoff_path: /Users/tom/Dev/projects/underlay/docs/handoffs/20260825-174520-g09-021-monorepo-contract-authority.md
base_required: pushed-main
tags: [coordination, handoff, worker, pr, g09, monorepo]
---
...
- **Planning artifacts included at the base:**
  ...
  `docs/roadmaps/g09/021-monorepo-contract-authority.md`, and
  `docs/logs/2026-08/25-174056-monorepo-rollout-compiled.md`.
- **Worker branch label:** `worker/g09-021-monorepo-contract-authority`.
...
- **Ready roadmaps, in order:**
  `docs/roadmaps/g09/021-monorepo-contract-authority.md` only.
- **Allowed runway:** `g09.021` only.
- **Remaining roadmap budget:** one roadmap.
- **Dispatch topology:** serial. `g09.022` remains blocked pending review and
  merge of this PR.
- **Parallel safety check:** no parallel Underlay docs worker is allowed because
  `g09.022` and `g09.023` overlap the same authority surfaces.
...
Please keep this run inside `g09.021`:
...
- **Out of scope:** second-tier guide cleanup in `g09.022`, conformance code in
  `g09.023`, Acowtancy or other consumer edits, Underlay Build skill
...
- **Planning lineage:** the 2026-08-17 Northstar refresh opened `g09` for
...
- **Why the roadmap is ready:** the strict spec records the topology, root manifest,
...
- **Open tension:** Acowtancy's README still contains obsolete `file:` prose.
  That is known evidence drift owned by `g09.024`, not permission to weaken the
...
Then read `AGENTS.md`, the strict spec, `g09/README.md`, the `g09.021` roadmap, and
...
5. Read the active spec, milestone, assigned roadmap, `AGENTS.md`, and canonical
   refs.
6. Run `effigy tasks` and `effigy doctor`. The planning base recorded a
   ...
   `PAPERCUTS.md`; do not silently fix or attribute those findings to this roadmap.

### While you work

- Execute only `g09.021`.
...
1. Run `effigy qa:docs`, `effigy qa:northstar`, `effigy health`, the targeted
   active-file `rg` proof required by the roadmap, and `git diff --check`.
2. Update `g09.021` and the batch log with actual evidence. Keep `g09.022`
...
checks, and changed files against the strict spec and `g09.021`. If the same
...
Merge remains operator-authorised. Closeout refs are the `g09.021` roadmap, its
...
blocker and stop. Do not make `g09.022` look ready before the orchestrator has
