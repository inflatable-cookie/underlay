---
title: g10.003 workspace-shape conformance worker handoff
...
created: 2026-08-25
updated: 2026-08-25
handoff_path: /Users/tom/Dev/projects/underlay/docs/handoffs/20260825-201515-g10-003-workspace-shape-conformance.md
base_required: pushed-main
tags: [coordination, handoff, worker, pr, g10, monorepo, conformance]
---

## What This Thread Was Doing

The maintainer chose Acowtancy's single-repository workspace as Underlay's only
supported consumer shape. `g10.001` made that shape normative. `g10.002`
...
- **Planning checkout:** clean after the `g10.002` closeout push.
- **Worker mode:** implementation worker dispatched by the orchestrator; this
  handoff activates the worker-only worktree preflight.
- **Planning artifacts included at the base:** the active strict spec, completed
  `g10.001` and `g10.002` cards and logs, ready `g10.003`, and aligned roadmap,
  contract, spec, architecture, and log front doors.
- **Worker branch label:** `worker/g10-003-workspace-shape-conformance`.
...
- **Ready cards, in order:**
  `docs/roadmaps/g10/batch-cards/003-workspace-shape-conformance.md` only.
- **Allowed runway:** `g10.003` only.
- **Remaining card budget:** one card.
- **Dispatch topology:** serial. `g10.004` remains blocked pending review and
  operator-authorized merge of this PR.
- **Parallel safety check:** no other ready lane exists. `g10.004` consumes the
...
- **Required validation:** inspect `effigy test --plan`, run the focused
  script/fixture tests added by this card, prove the check accepts Acowtancy,
...
Please keep this run inside `g10.003`:
...
- **In scope:** one generic consumer workspace-shape checker; its distributable
  ...
  Effigy integration guidance; and direct card/log/front-door evidence updates.
- **Out of scope:** security-policy rules in
  `scripts/check-consumer-conformance.sh`; changes to Effigy's schema; Acowtancy
  README repair (`g10.004`); Underlay Reference or other consumer normalization
  (`g10.005`–`g10.009`); workflow edits; release work; and unrelated contract,
...
- Keep the checker product-agnostic. Discover workspace package names and
  ...
  names.
- Enforce the ready card and contract `024`: one Git root with no nested
...
- **Planning lineage:** the strict rollout is serial through `g10.005`.
  `g10.001` owns the topology, `g10.002` owns the active narrative, and this
  card owns the mechanical shape gate. Consumer edits start after this gate is
  reviewed.
- **Why the card is ready:** contract `024` and the strict spec settle every
  shape rule; Acowtancy supplies a live compliant proof; the fleet baseline in
  the spec supplies failing cases; and the card names acceptance, validation,
  and stop boundaries.
...
Then read `AGENTS.md`, the strict spec, `g10/README.md`, the `g10.003` card,
...
5. Read the active spec, milestone, assigned card, `AGENTS.md`, completed
   `g10.002` log, and canonical refs.
6. Run `effigy tasks` and `effigy doctor`. Record the known Doctor debt without
   widening this card. Run `effigy graph status --json`; refresh the index before
   using graph results when it reports stale or degraded state.

### While you work

- Execute only `g10.003`.
...
2. Update `g10.003` with actual evidence and create one batch log under
   `docs/logs/2026-08/`. Set the card/front doors to `in review`; keep
   `g10.004` blocked because the orchestrator owns promotion after review and
   merge.
...
and changed files against the strict spec, contract `024`, and `g10.003`. If the
...
Merge remains operator-authorized. Closeout refs are the `g10.003` card, its
execution log, `g10/README.md`, the strict spec, contracts `120` and `121`, the
...
blocker and stop. Do not make `g10.004` look ready before orchestrator review
