---
title: g10.002 migration contract and active guide normalization worker handoff
...
created: 2026-08-25
updated: 2026-08-25
handoff_path: /Users/tom/Dev/projects/underlay/docs/handoffs/20260825-184027-g10-002-active-guide-normalization.md
base_required: pushed-main
tags: [coordination, handoff, worker, pr, g10, monorepo, docs]
---

## What This Thread Was Doing

The maintainer chose Acowtancy's single-repository workspace as Underlay's only
supported consumer shape. `g10.001` replaced the old polyrepo-compatible
...
- **Planning artifacts included at the base:** the strict spec, the completed
  `g10.001` card and log, the ready `g10.002` card, and refreshed roadmap,
  contract, spec, architecture, and log front doors.
- **Worker branch label:** `worker/g10-002-active-guide-normalization`.
...
- **Ready cards, in order:**
  `docs/roadmaps/g10/batch-cards/002-active-guide-normalization.md` only.
- **Allowed runway:** `g10.002` only.
- **Remaining card budget:** one card.
- **Dispatch topology:** serial. `g10.003` remains blocked pending review and
  operator-authorised merge of this PR.
- **Parallel safety check:** no parallel Underlay docs or conformance worker is
  allowed because `g10.003` depends on the normalized authority produced here.
...
Please keep this run inside `g10.002`:
...
- **Out of scope:** workspace-shape conformance code (`g10.003`), Acowtancy
  evidence repair (`g10.004`), all consumer repository edits, Underlay Build
...
- **Planning lineage:** the strict monorepo rollout is a ten-card serial runway
  through `g10.005`; consumer-specific cards may fan out only after the
  reference fixture merges. `g10.001` is complete at merge commit
  `ec21e51cc918284fd9b306144c8d44a2ce1cae96`.
- **Why the card is ready:** contract `024` now owns the topology, root manifest,
  dependency, lockfile, and Effigy rules. Review evidence settled the contract
  `021` selector correction. The card names every remaining active-doc surface,
...
- **Pre-existing health debt:** `effigy doctor` reports the unsupported
  ...
  in `PAPERCUTS.md`. Do not attribute or silently fix it in this card.
...
Then read `AGENTS.md`, the strict spec, `g10/README.md`, the `g10.002` card, the
completed `g10.001` log, and the canonical refs named above. Inspect the live
...
1. Run `effigy qa:docs`, `effigy qa:northstar`, `effigy health`, the targeted
   active-doc `rg` proof required by the card, and `git diff --check`.
2. Update `g10.002` with actual evidence and create one batch log under
   `docs/logs/2026-08/`. Set the card/front doors to `in review`; keep
   `g10.003` blocked because the orchestrator owns promotion after review and
   merge.
...
checks, and changed files against the strict spec and `g10.002`. If the same
...
Merge remains operator-authorised. Closeout refs are the `g10.002` card, its
...
blocker and stop. Do not make `g10.003` look ready before orchestrator review and
