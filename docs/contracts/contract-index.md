...
|---|---|---|---|
| [001-working-rules.md](./001-working-rules.md) | active | repo delivery rules | records sequential mode with no active generation after `g09` closure |
...
| [021-database-migration-and-schema-workflow.md](./021-database-migration-and-schema-workflow.md) | active | durable schema migration layout, dev overlay separation, reset/replay loop, migration proof posture | five API packages, `070`, `120`, migration usage policy | assessed by `g09.035`; conforming after `g09.038`–`g09.044` |
| [022-testing-posture-and-shared-harnesses.md](./022-testing-posture-and-shared-harnesses.md) | active | minimum and strong proof posture for API/admin/front packages plus shared harness usage | five app families, `120`, shared harness code | assessed by `g09.036`; conforming after `g09.037`–`g09.044`; fixed-schema whole-app suites app-owned |
| [023-release-and-compatibility-rollout.md](./023-release-and-compatibility-rollout.md) | active | fleet rollout order, released Git-tag consumer pins, compatibility windows, upgrade notes, and retirement proof for shared changes | `001`, `024`, `027`, `111`, `190`, `g01.031`, five-consumer rollout evidence | `g09.060` complete after normalizing path/file teaching onto released tags |
...
| [027-api-canonical-path-cutovers-and-compatibility-retirement.md](./027-api-canonical-path-cutovers-and-compatibility-retirement.md) | active | canonical-path migration, compatibility aliases, client/server cutover order, retirement policy | `026`, `118`, `g05.009`, `g09.057`–`g09.058` alias-retirement evidence | assessed by `g09.057`; conforming after all three `g09.058` repairs |
| [028-runtime-surface-and-openapi-maturity-levels.md](./028-runtime-surface-and-openapi-maturity-levels.md) | active | runtime maturity ladder for health, metrics, OpenAPI JSON, and Swagger exposure | `025`, `026`, endpoint-family matrix, five-site runtime evidence | assessed by `g09.057`; conforming at declared level A, B, and C profiles |
...
| [031-config-and-secrets.md](./031-config-and-secrets.md) | active | config classes, env naming, typed-config layering, local secret posture, and bootstrap boundary | `120-configuration`, `121-consumer-config-rollout-kit`, `024`, `030`, five consumer API setups | settles where env ends, typed config begins, and how normal apps should keep config and secrets from drifting into folklore |
| [032-openapi-quality-and-declaration.md](./032-openapi-quality-and-declaration.md) | active | minimum versus strong OpenAPI declaration coverage, envelope typing, helper/status schema posture, and anonymous-object quality bar | `028`, `115`, `118`, `119`, shared OpenAPI wrappers, five-site route declaration evidence | settles what good-enough OpenAPI means once an app exposes it and stops route documentation quality from drifting app by app |
...
| [051-media-library-fleet-capability-policy.md](./051-media-library-fleet-capability-policy.md) | active | fleet-wide requirement and classification policy for the admin media family | `040`, `050`, `110`, `111`, media capability matrix, `g05.004`, `g05.019` | settles whether all five consumer apps must own the full media quartet and makes missing media capability auditable drift instead of open interpretation |
...
| [070-nightfire-and-migration-systems.md](./070-nightfire-and-migration-systems.md) | active | standalone Nightfire Rust/TS content model plus Underlay integration and migration-core | standalone `nightfire`, temporary `underlay-nightfire`/TS facades, `underlay-migration-core` | settles the dual-language content authority, compatibility boundary, and migration discipline |
...
| [119-helper-search-and-lookup-route-catalogue.md](./119-helper-search-and-lookup-route-catalogue.md) | active | helper-route naming, lookup/search/suggest/detect/status catalogue, bounded helper envelope rules | `020`, `026`, `118`, five-site helper cleanup evidence | settles the repeated non-resource helper family so lookup/search/status routes stop drifting as one-off exceptions |
...
| [122-rust-public-api-inventory.md](./122-rust-public-api-inventory.md) | active | Rust public API inventory and first platform-contract migration gates | `g06.001`, Rust crate export scan, five-consumer dependency evidence | classifies stable, adapter, internal, candidate-type, and candidate-remove surfaces before the typed-boundary migration proceeds |
...
- the next repair work can be expressed as bounded roadmaps instead of
  exploratory drift
