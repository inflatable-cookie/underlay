...
| `underlay-ai-runtime`, `underlay-suggestions` | stable | Covered by `080`. `RetryConfig`, `CircuitBreakerConfig`, and `RouteChainConfig` fields are private as of `g06.211`; consumers must use defaults, builders, and accessors. |
| `underlay-nightfire` | stable → deprecation facade in `g12` | Card 278 moves the unchanged generic implementation to standalone crate `nightfire`; Card 274 replaces this package with a thin re-export facade. Preserve the current public API, including private `MultiConfig` fields plus constructors/builders/accessors from `g06.215`, until caller inventory authorizes retirement. |
