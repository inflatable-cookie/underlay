...
| `underlay-auth` | `AuthProvider`, extractors, principals, role sets, repository traits, auth/session/credential/user domain rows, optional Postgres state store, optional password hashing | stable | App-facing auth contract. Keep provider/repository traits here rather than in provider-specific crates. |
...
| `underlay-media` | media domain IDs/entities/inputs, `MediaRepository`, `MediaRepositoryExt`, storage-key helpers, sync/extractor helpers, optional Nightfire bridge, optional Postgres repo/config, optional rendition service | stable + adapter + candidate-type | Domain and repository traits are stable. Postgres config table names should continue toward typed table config. Adapter internals stay private. |
...
| `underlay-jobs` | job types, handler/store traits, runner/registry, event hub, dead-letter store, optional Postgres repositories, scheduler/outbox modules, migration SQL constants | stable + adapter | Traits are app-facing. Postgres repositories and migration SQL constants are adapter surfaces. Do not couple job runtime policy to app-specific queues. |
