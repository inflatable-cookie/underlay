...
| `underlay-http` | response helpers, `ApiError`, query parsing, pagination, CORS, context, caching, cookie helpers, feature-gated error logging / embed / OpenAPI helpers | stable + candidate-type | Query parsing is stable app-facing surface used through both `underlay_http::query::*` and crate-root re-exports; any internal split must preserve sort/filter parsing semantics, `WhereBuilder`, `FieldMapping`, and the exported `field_mapping!` macro path. Cookie config is stable. Cookie name/path/domain/value internals should become typed construction boundaries. Feature-gated operational modules remain app-facing only when enabled. |
