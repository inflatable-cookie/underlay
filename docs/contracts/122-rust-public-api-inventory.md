...
| `underlay-db` | pool config, migration helpers, schema helpers, SQL-dir runner, pagination, existence checks, media DB enums, identifier helpers | stable + candidate-type | Identifier helpers are the first safety seam. Promote raw `&str` schema/table inputs toward typed identifiers in `g06.002` / `g06.005`. |
...
| `underlay-blob` | `BlobAdapter`, `NoopAdapter`, upload/download/object types, `MediaConfig`, S3 adapter, local adapter | stable + adapter + candidate-type | Adapter trait and S3 adapter are stable. `LocalAdapter` is a narrow utility seam. Object keys should move behind a typed `BlobObjectKey`. |
| `underlay-media` | root media domain IDs/entities/inputs, `MediaRepository`, `MediaRepositoryExt`, `MediaError`, `MediaResult`; module-owned `storage`, `sync`, `nightfire`, and `renditions` helper surfaces | stable + candidate-type | Domain and repository traits are stable at root. Helper families are module-owned as of `g06.020`. The Postgres adapter moved to `underlay-media-postgres` in `g06.021`. |
| `underlay-media-postgres` | `PostgresMediaRepository`, `PostgresMediaConfig`, private SQL operation modules and row mappings | adapter + candidate-type | Concrete media Postgres adapter. Config table names should continue toward typed table config. Adapter internals stay private. |
...
| `underlay-audit` | audit row/action/filter, typed `AuditTable`, typed query/append helpers, retained raw-string wrappers | stable + candidate-type | Typed table config is the preferred path. Table-name string input is retained compatibility until a deprecation roadmap retires it. |
| `underlay-security-alerts` | alert config/types, detector, typed login-attempt and alert-event tables, typed SQL helpers, retained raw-string wrappers | stable + candidate-type | Typed table config is the preferred path. Table-name string inputs are retained compatibility until a deprecation roadmap retires them. |
...
Status: complete in `g06.006` and `g06.007`.
