...
| `underlay-db` | pool config, migration helpers, schema helpers, SQL-dir runner, pagination, typed existence checks, media DB enums, identifier helpers | stable + candidate-type | Identifier helpers are the first safety seam. `TypedExistsCheck` and typed value helpers are the retained existence-check surface; raw `ExistsCheck` and raw value helpers were removed in `g06.033`. Pagination is stable app-facing module surface; consumers import `underlay_db::pagination::*` directly, so any internal split must preserve the `pagination` front door, exported item names, cursor encoding, response shapes, and SQL helper behavior. |
