...
| `underlay-media-postgres` | `PostgresMediaRepository`, `PostgresMediaConfig`, private SQL operation modules and row mappings | adapter + candidate-type | Concrete media Postgres adapter. `PostgresMediaConfig` is the last known runtime dynamic identifier config that still stores public raw string fields; `g06.036` should move its internals to typed table config while preserving practical construction ergonomics. Adapter internals stay private. |
