...
| `underlay-ai-runtime`, `underlay-suggestions`, `underlay-nightfire` | stable | Covered by `080` and `070`; not part of the first platform-boundary migration. `RetryConfig`, `CircuitBreakerConfig`, and `RouteChainConfig` fields are private as of `g06.211`; consumers must use defaults, builders, and accessors. |
