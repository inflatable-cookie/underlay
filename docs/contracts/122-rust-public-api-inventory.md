...
| `underlay-media` | root media domain IDs/entities/inputs, `MediaRepository`, `MediaRepositoryExt`, `MediaError`, `MediaResult`, re-exported `BlobObjectKey`; module-owned `storage`, `sync`, `nightfire`, and `renditions` helper surfaces | stable + candidate-type | Domain and repository traits are stable at root. Stored object-key fields now use `BlobObjectKey`, and `underlay-media-postgres` parses database strings during row mapping. Helper families are module-owned as of `g06.020`. Storage key helpers now expose `BlobObjectKey` generation while retaining string helpers. The Postgres adapter moved to `underlay-media-postgres` in `g06.021`. |
