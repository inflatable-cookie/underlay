...
| `underlay-db` | pool config, migration helpers, schema helpers, SQL-dir runner, pagination, typed and compatibility existence checks, media DB enums, identifier helpers | stable + candidate-type | Identifier helpers are the first safety seam. `TypedExistsCheck` is the preferred composite existence helper; raw `ExistsCheck` and raw value helpers remain compatibility surface pending deprecation decision. |
