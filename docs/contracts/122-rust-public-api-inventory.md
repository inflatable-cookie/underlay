...
| `underlay-audit` | audit row/action/filter, typed `AuditTable`, typed query/append helpers, deprecated raw-string wrappers | stable + candidate-remove | Typed table config is the preferred path. Table-name string input remains only as deprecated compatibility. |
| `underlay-security-alerts` | alert config/types, detector, typed login-attempt and alert-event tables, typed SQL helpers, deprecated raw-string wrappers | stable + candidate-remove | Typed table config is the preferred path. Table-name string inputs remain only as deprecated compatibility. |
