...
| `underlay-blob` | `BlobAdapter`, `NoopAdapter`, upload/download/object types, `BlobObjectKey`, `MediaConfig`, S3 adapter, local adapter | stable + adapter + candidate-type | Adapter trait and S3 adapter are stable. `LocalAdapter` is a narrow utility seam. Request constructors already accept `BlobObjectKey`; adapter trait methods still accept raw `&str` for compatibility. |
