...
| `underlay-blob` | `BlobAdapter`, `BlobAdapterObjectKeyExt`, `NoopAdapter`, upload/download/object types, `BlobObjectKey`, `MediaConfig`, S3 adapter, local adapter | stable + adapter + candidate-type | Adapter trait and S3 adapter are stable. `LocalAdapter` is a narrow utility seam. Request constructors and typed adapter extension methods accept `BlobObjectKey`; core adapter trait methods still accept raw `&str` for compatibility. |
| `underlay-media` | root media domain IDs/entities/inputs, `MediaRepository`, `MediaRepositoryExt`, `MediaError`, `MediaResult`; module-owned `storage`, `sync`, `nightfire`, and `renditions` helper surfaces | stable + breaking-candidate | Domain and repository traits are stable at root, but stored object-key fields remain candidate-type. Helper families are module-owned as of `g06.020`. Storage key helpers now expose additive feature-gated `BlobObjectKey` generation while retaining string helpers. The next media row-field cleanup should parse stored object keys into `BlobObjectKey` at the repository/domain boundary and roll through consumers as a controlled breaking change. The Postgres adapter moved to `underlay-media-postgres` in `g06.021`. |
