...
| `underlay-media` | root media domain IDs/entities/inputs, `MediaRepository`, `MediaRepositoryExt`, `MediaError`, `MediaResult`; module-owned `storage`, `sync`, `nightfire`, and `renditions` helper surfaces | stable + candidate-type | Domain and repository traits are stable at root. Helper families are module-owned as of `g06.020`. Storage key helpers should add typed `BlobObjectKey` generation before any string-helper retirement. The Postgres adapter moved to `underlay-media-postgres` in `g06.021`. |
