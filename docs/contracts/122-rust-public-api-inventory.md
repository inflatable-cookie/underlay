...
| `underlay-blob` | `BlobAdapter`, `NoopAdapter`, upload/download/object types, `BlobObjectKey`, `MediaConfig`, S3 adapter, local adapter | stable + adapter + candidate-type | Adapter trait and S3 adapter are stable. `LocalAdapter` is a narrow utility seam. Request constructors already accept `BlobObjectKey`; adapter trait methods still accept raw `&str` for compatibility. Next typed adapter work should be additive extension methods, not trait-signature churn. |
| `underlay-media` | root media domain IDs/entities/inputs, `MediaRepository`, `MediaRepositoryExt`, `MediaError`, `MediaResult`; module-owned `storage`, `sync`, `nightfire`, and `renditions` helper surfaces | stable + candidate-type | Domain and repository traits are stable at root. Helper families are module-owned as of `g06.020`. Storage key helpers now expose additive feature-gated `BlobObjectKey` generation while retaining string helpers. The Postgres adapter moved to `underlay-media-postgres` in `g06.021`. |
