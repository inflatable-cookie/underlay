...
| `underlay-email`, `underlay-events`, `underlay-ratelimit`, `underlay-observability`, `underlay-metrics` | stable + adapter | Covered by `060`, `020`, and operational contracts. Review only when affected by a gate. `RateLimitConfig` fields are private as of `g06.198`; consumers must use constructors and accessors. `ObservabilityConfig` fields are private as of `g06.203`; consumers must use builders and accessors. |
