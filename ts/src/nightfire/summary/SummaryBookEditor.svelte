<script lang="ts">
  ...
  export let onChange: (block: SummaryBookBlock) => void;

  function ensureBlock(b: SummaryBookBlock | undefined): SummaryBookBlock {
    const block =
      b ??
      ({
        type: "summary.book",
    ...
    if (!block.data) block.data = { pages: [] };
...
<div class="summary-book-editor">
  <p class="hint">
    Configure a multi-page “book” summary. Each page becomes a card in the
    student view.
  </p>
