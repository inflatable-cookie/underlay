<script lang="ts">
  ...
  import { Button, Callout, Field, FormActions, Tabs, TextInput } from "@poodle/svelte-primitives";
...
{#if hasTotpSetup}
  ...
  <form onsubmit={handleSubmit} class="underlay-two-factor-step">
    ...
    {#if activeMethod === "totp"}
      <TotpInput bind:value={code} label="2FA code" disabled={loading} error={error ?? undefined} />
    {:else if activeMethod === "backup"}
      ...
      {/if}
    {:else if emailCodeRequested}
      <TotpInput bind:value={code} label="Email code" disabled={loading} error={error ?? undefined} />
    {:else}
