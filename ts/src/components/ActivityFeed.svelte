<script lang="ts" module>
  /**
   * Activity feed entry representing an audit log action.
   */
...
<script lang="ts">
  ...
  interface Props {
    ...
    error?: string | null;
    /** Message shown when no activities */
    emptyMessage?: string;
    /** Custom action icon snippet - receives (actionType: ActionType) */
    actionIcon?: Snippet<[ActionType]>;
    /** Custom entry details snippet - receives (entry: ActivityEntry) */
    entryDetails?: Snippet<[ActivityEntry]>;
    /** Custom action-to-type mapping function */
    getActionType?: (action: string) => ActionType;
  }
  ...
  function defaultGetActionType(action: string): ActionType {
    switch (action) {
      case "create":
      case "created":
        return "create";
